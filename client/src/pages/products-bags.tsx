...
export default function ProductsBags() {
  const addItem = useCartStore(state => state.addItem);
  const { toast } = useToast();

  const { data: products, isLoading } = useQuery<Product[]>({
    queryKey: ["/api/products/category/bags"],
  });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-serif font-bold text-khaista-charcoal mb-4">
            Afghan Bags Collection
          </h1>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Explore our beautiful collection of traditional Afghan bags, featuring vibrant embroidery and authentic craftsmanship.
          </p>
        </div>
