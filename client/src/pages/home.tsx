...
// Load featured ids and join with products
async function fetchFeatured(): Promise<ProductForCard[]> {
  const [all, ids] = await Promise.all([
    fetchAllProducts(),
    fetch(BASE + "api/featured-products.json").then((r) => r.json() as Promise<{ id: string }[]>),
  ]);
      ...
      <section className="py-8 bg-gradient-to-r from-amber-50 to-orange-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <p className="text-lg text-khaista-charcoal">
            <span className="font-semibold">Proud partner of the Afghanistan Women's Council</span>{" "}
            - Supporting Afghan women's financial independence through traditional craftsmanship
          </p>
          <Link href="/about">
            <Button
              variant="outline"
              className="mt-4 border-khaista-pink text-khaista-pink hover:bg-khaista-pink hover:text-white"
            >
